...
function InfoRow({ label, value, mono }) {
  ...
  return (
    <div>
      <div className="text-[11px] uppercase tracking-widest mb-1" style={{ color: '#8888a0', fontFamily: "'Poppins',sans-serif" }}>{label}</div>
...
function ReviewModal({ review, criteria, onSave, onClose }) {
  ...
  return (
    <Modal open onClose={onClose} title="Score This Call"
      ...
      {criteria.length === 0 ? (
        <div className="text-center py-6 text-sm" style={{ color: '#8888a0' }}>No criteria defined. Go to QA Criteria to add some.</div>
      ) : (
        <>
          <div className="flex items-center justify-between mb-3">
            <div className="font-bold text-sm" style={{ color: '#1a1a2e', fontFamily: "'Poppins',sans-serif" }}>QA Criteria Scoring</div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 mt-0.5">
                    ...
                    <span className="text-[10px] px-1.5 py-px rounded"
                      style={{ background: '#dcdce0', border: '1px solid #c8c8ce', color: '#8888a0' }}>{c.weight ?? 100}%</span>
                  </div>
                </div>
          ...
          <div className="rounded-xl border p-4"
            style={{ background: score === null ? '#ebebee' : isPassing ? '#e6f9ee' : '#fde8ec', border: `1px solid ${score === null ? '#d0d0d6' : isPassing ? '#a8ecc0' : '#f8c0cc'}` }}>
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-semibold uppercase tracking-widest" style={{ color: '#8888a0', fontFamily: "'Poppins',sans-serif" }}>Score Preview</span>
              <div className="flex items-center gap-2">
                {score !== null && <span className="font-bold text-xl" style={{ color, fontFamily: "'Poppins',sans-serif" }}>{score}%</span>}
                {score !== null && (
                ...
                {score === null && <span className="text-[11px]" style={{ color: '#8888a0' }}>Score criteria above</span>}
              </div>
            </div>
            ...
            <div className="flex justify-end mt-1">
              <span className="text-[9px]" style={{ color: '#8888a0', fontFamily: "'Poppins',sans-serif" }}>Pass threshold: 60%</span>
            </div>
          </div>
        </>
      )}
...
function DetailModal({ review, criteria, onClose, onFlag }) {
  ...
  return (
    <Modal open title="Call Review Details" onClose={onClose}
      ...
      {criteria.length > 0 && (
        <>
          <div className="font-bold text-sm mb-3" style={{ color: '#1a1a2e', fontFamily: "'Poppins',sans-serif" }}>Criteria Scores</div>
    ...
    <Modal open onClose={onClose} title="Flag for Re-review"
      footer={
        <>
          ...
          <Btn onClick={() => { if (!reason.trim()) { emitToast('Please enter a reason', 'error'); return } onConfirm(reason) }}
            style={{ background: '#d97706' }}>
            Flag Call
          </Btn>
        </>
      }>
      <p className="text-[13px] mb-4 leading-relaxed" style={{ color: '#505060', fontFamily: "'Poppins',sans-serif" }}>
        This will mark the call as disputed and request a second review.
        <br />
        <span className="font-semibold" style={{ color: '#1a1a2e' }}>{review.agentName}</span>
        {review.sid && <span className="font-mono text-[12px] ml-2" style={{ color: '#8888a0' }}>{review.sid}</span>}
      </p>
      <div>
        ...
        <textarea
          ...
          placeholder="Why should this call be reviewed again?"
          autoFocus
          style={{
            background: '#f5f5f8', border: '1px solid #d0d0d6', borderRadius: 8,
  ...
  return (
    <Modal open onClose={onClose} title={`Assign Reviewer — ${count} call${count > 1 ? 's' : ''}`}
      footer={
        <>
          <Btn variant="ghost" onClick={onClose}>Cancel</Btn>
          <Btn onClick={() => { if (!reviewer.trim()) { emitToast('Enter a reviewer name', 'error'); return } onConfirm(reviewer.trim()) }}>
            Assign
          </Btn>
        </>
      }>
  ...
  return (
    <Modal open onClose={onClose} title="Remove Call"
      ...
      <p className="text-sm leading-relaxed" style={{ color: '#505060', fontFamily: "'Poppins',sans-serif" }}>
        Are you sure you want to remove this call from the log?<br />
        <span className="font-semibold" style={{ color: '#1a1a2e' }}>{review.agentName}</span>
        {review.sid && <span className="font-mono text-[12px] ml-2" style={{ color: '#8888a0' }}>{review.sid}</span>}<br />
        <span className="text-[12px]" style={{ color: '#8888a0' }}>This cannot be undone.</span>
      </p>
    </Modal>
  )
}
...
export default function CallLog({ state, updateReview, deleteReview, bulkDeleteReviews, bulkAssignReviewer, flagForDispute }) {
  ...
  const actionBtnStyle = (color = '#505060', bg = '#dcdce0', border = '#c8c8ce') => ({
    ...
    fontFamily: "'Poppins',sans-serif", background: bg,
    ...
    <div className="p-7 animate-fadeIn">
      ...
      <Panel className="mb-4">
        <div className="px-5 py-3.5 flex gap-3 items-center flex-wrap">
          ...
          />
          <select value={filterResult} onChange={(e) => setFilterResult(e.target.value)} style={{ ...selectStyle, width: 140 }}>
            <option value="">All Results</option>
            <option value="pass">Pass</option>
            <option value="fail">Fail</option>
            <option value="pending">Pending</option>
          </select>
          <select value={filterAgent} onChange={(e) => setFilterAgent(e.target.value)} style={{ ...selectStyle, width: 180 }}>
      ...
      {someSelected && (
        <div className="flex items-center gap-3 px-4 py-3 rounded-xl mb-4"
          style={{ background: '#e8f0ff', border: '1px solid #c8d4f0' }}>
          <span className="text-[13px] font-semibold" style={{ color: '#2563eb', fontFamily: "'Poppins',sans-serif" }}>
          ...
          : <table className="w-full border-collapse">
              <thead>
                <tr>
                  ...
                  {['Agent', 'Call Date', 'SID', 'Talk Time', 'Result', ''].map((h) => (
                ...
                {filtered.map((r) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-[12px] whitespace-nowrap" style={{ color: '#505060' }}>{r.callDate}</td>
                      <td className="px-4 py-3 font-mono text-[11px] max-w-[140px] truncate" title={r.sid} style={{ color: '#505060' }}>
                        {r.sid || <span style={{ color: '#a0a0b0' }}>—</span>}
                      </td>
                      <td className="px-4 py-3 font-mono text-[11px]" style={{ color: '#505060' }}>
                        {r.talkDuration || <span style={{ color: '#a0a0b0' }}>—</span>}
                      </td>
      ...
      {detail && (
        <DetailModal review={detail} criteria={state.criteria}
          onClose={() => setDetail(null)}
          onFlag={handleFlag} />
      )}
      {flagging && (
        <DisputeModal review={flagging}
          onConfirm={handleConfirmFlag}
          onClose={() => setFlagging(null)} />
      )}
      {confirmDelete && (
        <ConfirmModal review={confirmDelete} onConfirm={handleDelete} onClose={() => setConfirmDelete(null)} />
      )}
      {bulkAssign && (
        <BulkAssignModal count={selected.size} onConfirm={handleBulkAssign} onClose={() => setBulkAssign(false)} />
      )}
      {confirmBulkDel && (
        <Modal open onClose={() => setConfirmBulkDel(false)} title="Delete Selected Calls"
          ...
          <p className="text-sm leading-relaxed" style={{ color: '#505060', fontFamily: "'Poppins',sans-serif" }}>
            Are you sure you want to permanently delete <strong style={{ color: '#1a1a2e' }}>{selected.size} call{selected.size > 1 ? 's' : ''}</strong>?
            <br /><span className="text-[12px]" style={{ color: '#e11d48' }}>This cannot be undone.</span>
          </p>
        </Modal>
      )}
    </div>
